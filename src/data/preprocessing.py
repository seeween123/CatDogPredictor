...
def copy_images(
    imgs, lbls, split, base_dir: str = "../dataset/Processed/cats_dogs_split"
):
...
def preprocess_data(
    ...
    cats = [
        os.path.join(source_dir, "Cat", f)
        for f in os.listdir(os.path.join(source_dir, "Cat"))
        if f.endswith(".jpg")
    ]
    dogs = [
        os.path.join(source_dir, "Dog", f)
        for f in os.listdir(os.path.join(source_dir, "Dog"))
        if f.endswith(".jpg")
    ]
